...
# Mixed precision compatible TopK metric
class MixedPrecisionTopK(TopKCategoricalAccuracy):
    """TopKCategoricalAccuracy that works with mixed precision by casting to float32"""
    def update_state(self, y_true, y_pred, sample_weight=None):
        # Cast predictions to float32 for compatibility with in_top_k op
...
def parse_args():
    ...
    parser.add_argument('--use-mixed-precision', action='store_true',
                        help='Enable mixed precision (float16) training - saves ~40%% memory but may cause NaN for some models')
...
def setup_gpu(args):
    ...
    # Disable XLA to avoid ptxas issues on RTX 5090
    ...
    if args.disable_gpu:
        ...
        # Enable mixed precision only if requested
        ...
        if gpus:
            try:
                ...
                if args.gpu < len(gpus):
                    ...
                    print(f"Memory growth enabled - will use available VRAM as needed")
                else:
...
def main():
    ...
    # Wrap optimizer with loss scaling if using mixed precision
    optimizer = Adam(learning_rate=args.learning_rate)
    if args.use_mixed_precision:
        optimizer = tf.keras.mixed_precision.LossScaleOptimizer(optimizer)
        print("Using loss scaling for mixed precision training")
    ...
    model.compile(
        ...
        metrics=[
            'accuracy',
            MixedPrecisionTopK(k=5, name='top_5_accuracy') if args.use_mixed_precision else TopKCategoricalAccuracy(k=5, name='top_5_accuracy'),
        ...
        jit_compile=False  # Disable XLA compilation (workaround for RTX 5090 ptxas issues)
    )
    ...
    print(f"***** BATCH SIZE: {args.batch_size} *****")  # DEBUG: Confirm batch size
