...
def build_writer_identification_model(input_shape, num_clusters, num_classes):
    ...
    x_resNet50 = x  # Shape: (None, 7, 7, 64)

    # Reshape for self-attention
    x_resNet50_flat = Reshape((49, 64))(x_resNet50)  # 7*7=49
    print('Shape after reshaping for self-attention (ResNet50 block):', x_resNet50_flat.shape)  # (None, 49, 64)

    # Apply Layer Normalization before Self-Attention
    x_resNet50_flat = layers.LayerNormalization()(x_resNet50_flat)

    # Self-Attention after ResNet50 block
    attn_layer1 = layers.MultiHeadAttention(num_heads=6, key_dim=32)  # Adjusted num_heads and key_dim
    x_attn1 = attn_layer1(query=x_resNet50_flat, key=x_resNet50_flat, value=x_resNet50_flat)
    print('Shape after self-attention ResNet50 block):', x_attn1.shape)  # (None, 49, 64)
    ...
    print('Shape after reshaping back ResNet50 block):', x_attn1_reshaped.shape)  # (None, 7, 7, 64)
    ...
    # Prepare the key and value for cross-attention from the ResNet50 self-attention output
    x_attn1_flat_proj = Dense(D_vlad, activation='relu', name='proj_attn1_flat')(x_resNet50_flat)  # (None, 49, 192)
