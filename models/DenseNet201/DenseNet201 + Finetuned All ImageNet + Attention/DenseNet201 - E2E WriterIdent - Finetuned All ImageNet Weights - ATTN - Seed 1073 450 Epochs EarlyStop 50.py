...
def build_writer_identification_model(input_shape, num_clusters, num_classes):
    # Base model using DenseNet201

    # Do note that removing weights means that
    # the default value for weights, which is 'imagenet'
    # will not be used

    # The default IS NOT weights=None
    # If we want to fully train from scratch
    # Then weights=None has to be specified

    # Now, we finetune ImageNet fully
    ...
    x = base_model(inputs)
    print('Shape after base_model:', x.shape)  # Expected: (None, 7, 7, 2048) for input_shape=(224,224,3)
    ...
    x_denseNet201 = x  # Shape: (None, 7, 7, 64)

    # Reshape for self-attention
    x_denseNet201_flat = Reshape((49, 64))(x_denseNet201)  # 7*7=49
    print('Shape after reshaping for self-attention (DenseNet201 block):', x_denseNet201_flat.shape)  # (None, 49, 64)

    # Apply Layer Normalization before Self-Attention
    x_denseNet201_flat = layers.LayerNormalization()(x_denseNet201_flat)

    # Self-Attention after DenseNet201 block
    attn_layer1 = layers.MultiHeadAttention(num_heads=6, key_dim=32)  # Adjusted num_heads and key_dim
    x_attn1 = attn_layer1(query=x_denseNet201_flat, key=x_denseNet201_flat, value=x_denseNet201_flat)
    print('Shape after self-attention (DenseNet201 block):', x_attn1.shape)  # (None, 49, 64)
    ...
    print('Shape after reshaping back (DenseNet201 block):', x_attn1_reshaped.shape)  # (None, 7, 7, 64)
    ...
    # Prepare the key and value for cross-attention from the DenseNet201 self-attention output
    x_attn1_flat_proj = Dense(D_vlad, activation='relu', name='proj_attn1_flat')(x_denseNet201_flat)  # (None, 49, 192)
...
checkpoint_path = f"{OUTPUT_PREFIX}ATTN_{BACKBONE}_Finetuned_All_ImageNet_E2E_WriterIdent_best_model_val_f1_{SEED}.keras"
...
periodic_checkpoint_path = f"{OUTPUT_PREFIX}ATTN_{BACKBONE}_Finetuned_All_ImageNet_E2E_WriterIdent_Last_Saved_Epoch_{SEED}.keras"
...
report_df.to_csv(f'{OUTPUT_PREFIX}ATTN_{BACKBONE}_Finetuned_All_ImageNet_classification_report_{SEED}.csv', index=True)

print(f"Final Classification Report saved as 'ATTN_{BACKBONE}_Finetuned_All_ImageNet_classification_report_{SEED}.csv'.")
...
with open(f'{OUTPUT_PREFIX}ATTN_{BACKBONE}_Finetuned_All_ImageNet_history_seed_{SEED}.pkl', 'wb') as f:
...
with open(f'{OUTPUT_PREFIX}ATTN_{BACKBONE}_Finetuned_All_ImageNet_test_metrics_seed_{SEED}.json', 'w') as f:
    json.dump(test_metrics, f)

# Save the elapsed time to a file
with open(f'{OUTPUT_PREFIX}ATTN_{BACKBONE}_Finetuned_All_ImageNet_elapsed_time_seed_{SEED}.txt', 'w') as f:
...
# plt.show()
plt.savefig(f'{OUTPUT_PREFIX}ATTN_{BACKBONE}_Finetuned_All_ImageNet_confusion_matrix_{SEED}.png')
# plt.close()
...
plt.ylabel('Accuracy')
plt.legend()
plt.savefig(f'{OUTPUT_PREFIX}ATTN_{BACKBONE}_Finetuned_All_ImageNet_accuracy_{SEED}.png')
plt.show()
plt.close()
...
plt.legend()
plt.savefig(f'{OUTPUT_PREFIX}ATTN_{BACKBONE}_Finetuned_All_ImageNet_top_5_accuracy_{SEED}.png')
plt.show()
plt.close()
...
plt.ylabel('Loss')
plt.legend()
plt.savefig(f'{OUTPUT_PREFIX}ATTN_{BACKBONE}_Finetuned_All_ImageNet_loss_{SEED}.png')
plt.show()
plt.close()
...
plt.ylabel('F1 Score')
plt.legend()
plt.savefig(f'{OUTPUT_PREFIX}ATTN_{BACKBONE}_Finetuned_All_ImageNet_f1_score_{SEED}.png')
plt.show()
plt.close()
...
plt.legend()
plt.savefig(f'{OUTPUT_PREFIX}ATTN_{BACKBONE}_Finetuned_All_ImageNet_precision_{SEED}.png')
plt.show()
plt.close()
...
plt.ylabel('Recall')
plt.legend()
plt.savefig(f'{OUTPUT_PREFIX}ATTN_{BACKBONE}_Finetuned_All_ImageNet_recall_{SEED}.png')
plt.show()
plt.close()
