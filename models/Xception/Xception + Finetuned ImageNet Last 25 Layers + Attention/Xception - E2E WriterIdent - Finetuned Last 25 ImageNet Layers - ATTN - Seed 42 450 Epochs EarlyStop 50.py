...
def build_writer_identification_model(input_shape, num_clusters, num_classes):
    ...
    x_xception = x  # Shape: (None, 7, 7, 64)

    # Reshape for self-attention
    x_xception_flat = Reshape((49, 64))(x_xception)  # 7*7=49
    print('Shape after reshaping for self-attention (Xception block):', x_xception_flat.shape)  # (None, 49, 64)

    # Apply Layer Normalization before Self-Attention
    x_xception_flat = layers.LayerNormalization()(x_xception_flat)

    # Self-Attention after Xception block
    attn_layer1 = layers.MultiHeadAttention(num_heads=6, key_dim=32)  # Adjusted num_heads and key_dim
    x_attn1 = attn_layer1(query=x_xception_flat, key=x_xception_flat, value=x_xception_flat)
    print('Shape after self-attention (Xception block):', x_attn1.shape)  # (None, 49, 64)
    ...
    print('Shape after reshaping back (Xception block):', x_attn1_reshaped.shape)  # (None, 7, 7, 64)
    ...
    # Prepare the key and value for cross-attention from the Xception self-attention output
    x_attn1_flat_proj = Dense(D_vlad, activation='relu', name='proj_attn1_flat')(x_xception_flat)  # (None, 49, 192)
